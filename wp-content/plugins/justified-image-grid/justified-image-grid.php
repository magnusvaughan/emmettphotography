...
text-align:left;",
			...
			if(!$case){
				...
				if($this->settings['take_over_ngg_tag'] === 'yes'){
					add_filter('the_content',array($this,'jig_detect_ngg_tag'));
				}
		...
		function jig_autoupdate_altapi_request($args){
			...
			if(function_exists('mcrypt_decrypt')){
				$response = unserialize(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5($encryption_key), base64_decode(wp_remote_retrieve_body($request)), MCRYPT_MODE_CBC, md5(md5($encryption_key))));
			}else{
				$response = unserialize(base64_decode(wp_remote_retrieve_body($request)));
			}
		...
		function jig_autoupdate_information($false, $action, $args){
			...
			if(empty($args->slug) || (!empty($args->slug) && $args->slug != $plugin_slug_v1 && $args->slug != $plugin_slug_v2)){
				return false;
			}
		...
		function head_begin(){
					...
					if(!empty($provider) && !empty($content_id)){
						...
						switch ($provider) {
							...
							case 'FB':
								...
								$photos_url = 'https://graph.facebook.com/v2.2/'.$content_id.'?fields=name,images&access_token='.$token;
								...
								$ext = '';
								if (preg_match('/.*\.(jpe?g|gif|bmp|webp)/im', $photo->images[0]->source, $regs)) {
									$ext = "&f=.".$regs[1];
								}
							...
							case 'FL':
								...
								if($this->settings['flickr_caching'] > 0){
									if(get_transient('jigfli_'.md5($photo_info_url.$this->settings['flickr_caching'])) == true){
										...
										$photo_info = unserialize($this->file_get_contents_curl($photo_info_url));
								...
								if($this->settings['flickr_caching'] > 0){
									if(get_transient('jigfli_'.md5($photo_sizes_url.$this->settings['flickr_caching'])) == true){
										...
										$photo_sizes = unserialize($this->file_get_contents_curl($photo_sizes_url));
			...
			if(preg_match("/(!.*\/(\d+?\/)?(https?:\/\/[\w\d\/.\-\?=]*\.(?:jpg|jpeg|png|gif|bmp)[\w\d\/.\-\?=]*)?)$/", $this->requestURI, $mediaURI) === 1){
				$this->custom_og_image = "<meta property='og:image' content='".$mediaURI[3]."' />\n";
				...
				$this->custom_og_image = "<meta property='og:image' content='".$mediaURI[1]."' />\n";
			...
			if($this->settings['conditional_script_loading'] == 'no'){
				...
				if($this->settings['load_bundled_lightbox'] == 'yes'){
					if(in_array('prettyphoto', $this->settings['scripts_to_load'])){
						wp_register_script('jig-prettyphoto', plugins_url('js/jquery.prettyphoto.custom-min.js', __FILE__), 'jquery', '3.1.5.'.self::PLUGIN_VERSION, true);
						wp_enqueue_script('jig-prettyphoto');
						wp_register_style('jig-prettyphoto-style', plugins_url('css/prettyphoto.css', __FILE__), false, '3.1.5.'.self::PLUGIN_VERSION);
						wp_enqueue_style('jig-prettyphoto-style');
					}
					if(in_array('colorbox', $this->settings['scripts_to_load'])){
						wp_deregister_script('colorbox');
						wp_register_script('colorbox', plugins_url('js/jquery.colorbox-min.js', __FILE__), 'jquery', '1.5.14', true);
						wp_enqueue_script('colorbox');
						wp_register_style('colorbox-style', plugins_url('css/colorbox'.$this->settings['colorbox_design'].'.css', __FILE__), false, '1.5.14');
						wp_enqueue_style('colorbox-style');
					}

					if(in_array('magnific', $this->settings['scripts_to_load'])){
						wp_deregister_script('magnific-popup');
						wp_register_script('magnific-popup', plugins_url('js/jquery.magnific-popup.min.js', __FILE__), 'jquery', '0.9.9', true);
						wp_enqueue_script('magnific-popup');
						wp_register_style('magnific-popup-style', plugins_url('css/magnific-popup.css', __FILE__), false, '0.9.9');
					...
					if(in_array('photoswipe4', $this->settings['scripts_to_load'])){
						...
						wp_register_script('photoswipe', plugins_url('js/photoswipe4-min.js', __FILE__), 'jquery', '4', true);
						wp_enqueue_script('photoswipe');
						wp_register_style('photoswipe-style', plugins_url('css/photoswipe4.css', __FILE__), 'jquery', '4');
				...
				if(in_array('dotdotdot', $this->settings['scripts_to_load'])){
					wp_enqueue_script('dotdotdot', plugins_url('js/jquery.dotdotdot.min.js', __FILE__), 'jquery', '1.6.16', true);			
				}
		...
		function jig_build_settings_page(){
			...
			<script type="text/javascript">
					...
					function jig_edit_preset_authority(){
						var button = jQuery("#jigEditPresetAuthority");
							jQuery("#jigPresetAuthorityHelp").toggle();

						if(!button.attr('data-active')){
							button.attr('data-active','yes');
							button.text("<?php _e('Exit preset authority', 'jig_td'); ?>");
							jQuery('#jigSettings').addClass('jigPa');

							jQuery.each(settings_flexible, function(index,setting){
								jQuery('#jigContext-'+setting).parent().parent().addClass('jigPaFlexible');
							});

							jQuery.each(settings_protected, function(index,setting){
								jQuery('#jigContext-'+setting).parent().parent().addClass('jigPaProtected');
							});

							jQuery.each(settings_public, function(index,setting){
								jQuery('#jigContext-'+setting).parent().parent().addClass('jigPaPublic');
							});
						}else{
							button.removeAttr('data-active');
							button.text("<?php _e('Edit preset authority', 'jig_td'); ?>");
							jQuery('.jigPaFlexible, .jigPaProtected, .jigPaPublic').removeClass('jigPaFlexible jigPaProtected jigPaPublic');
							jQuery('#jigSettings').removeClass('jigPa');
						}
					...
					function change_preset_authority(setting, group){
						if(group == 'settings_flexible'){
							var positionInSettingsPublic = jQuery.inArray(setting, settings_public);
							...
							var positionInSettingsFlexible = jQuery.inArray(setting, settings_flexible);
						...
						jQuery('#settings_flexible').val(settings_flexible.join());
						jQuery('#settings_public').val(settings_public.join());
					...
					function jig_toggle_fb_app_help(){
						jQuery('#jigFbAppHelp').fadeToggle(400);
					}
					function jig_toggle_fb_other_user_help(){
						jQuery('#jigFbOtherUserHelp').fadeToggle(400);

					}
					function jig_toggle_fli_api_help(){
						jQuery('#jigFliApiHelp').fadeToggle(400);
					}
					function jig_toggle_ig_app_help(){
						jQuery('#jigIgAppHelp').fadeToggle(400);
					}
					...
					function jig_purge_rss_caching(){
							...
							function(data) {
								jQuery("#jigRSSPurge").html(data['result']);
							},
							'json');
					}
					...
					function jig_on_demand_check_permissions(){
							...
							function(data) {
								...
								var worksImage = jQuery('<img src="'+jQuery("#jigTimthumbTester").attr('data-works')+'" width="200" height="50" />');
								worksImage.on('error', function(){
									jQuery("#jigTimthumbTester").empty();
								});
								jQuery("#jigTimthumbTester").empty().html(worksImage).css('background-image','url("'+jQuery("#jigTimthumbTester").attr('data-error')+'")');
					...
					jQuery(document).ready(function(jQuery) {
						...
						if(!(jQuery("#fb_app_id").val() != '' && jQuery("#fb_app_secret").val() != '')){
							jQuery('#jigFbWithAppOnly').html(<?php echo "'".'<div id="jigFbToAddUserHelpTitle">'.__('To add users or pages', 'jig_td').':</div><ol><li>'.__('Create a Facebook App', 'jig_td').'</li><li>'.__('Fill out the App ID and App Secret fields', 'jig_td').'</li><li>'.__('Click Save Changes', 'jig_td').'</li></ol>'."'"; ?>)
						}
						...
						addMiniSection('link_class',"<?php _e('Custom lightbox', 'jig_td'); ?>");
						addMiniSection('prettyphoto_social',"<?php _e('PrettyPhoto settings', 'jig_td'); ?>");
						addMiniSection('magnific_settings',"<?php _e('Other lightbox settings', 'jig_td'); ?>");
						...
						jQuery('#jigPresetSelect').on("change", function(){
							var selected = jQuery('#jigPresetSelect option:selected').val();
							if(selected !== 'default'){ // If a preset is selected
								jQuery('#jigApplyPreset').removeAttr('disabled');
							 
								if(selected.indexOf('c') === 0){
									// If it's custom
									jQuery('#jigDeletePreset, #jigSavePreset').show();
									jQuery('.jigNewPresetUI').hide();
									jQuery('#submitButton').val("<?php _e('Save to selected preset', 'jig_td'); ?>")
									jQuery('#saveToPresetField').removeAttr('disabled').val(selected);
									
								}else{ 
									// If it's normal
									jQuery('#jigDeletePreset, #jigSavePreset').hide();
									jQuery('.jigNewPresetUI').show();
									jQuery('#submitButton').val("<?php _e('Save Changes', 'jig_td'); ?>");
									jQuery('#saveToPresetField').attr('disabled','disabled');
								}
							}else{ // If no preset is selected (default dropdown state)
								jQuery('#jigApplyPreset').attr('disabled','disabled');
								jQuery('#jigDeletePreset, #jigSavePreset').hide();
								jQuery('.jigNewPresetUI').show();
								jQuery('#submitButton').val("<?php _e('Save Changes', 'jig_td'); ?>");
								jQuery('#saveToPresetField').attr('disabled','disabled');								
						...
						jQuery('#jigDeletePreset').on('click',function(){
						...
						jQuery('#jigApplyPreset').on('click',function(){
						...
						jQuery('#jigSavePreset').on('click',function(e){
							e.preventDefault();
							jQuery('#submitButton').trigger('click');
						})


						jQuery('#jigEditPresetAuthority').on('click',jig_edit_preset_authority);
						...
						jQuery('#jigSettings').on("click", ".jigPaFlexible th", function(event){
							jQuery(this).parent().removeClass('jigPaFlexible').addClass('jigPaPublic');
							var settingName = jQuery(this).next().find('.jigContextHelp').attr('id');
							...
							jQuery(this).parent().removeClass('jigPaPublic').addClass('jigPaFlexible');
							var settingName = jQuery(this).next().find('.jigContextHelp').attr('id');
							settingName = settingName.substr(11);
						...
						jQuery('#jigColorHelperField').minicolors({position: 'bottom right'});
						jQuery('#jigColorHelperField').on('click', function(){
							jQuery(this).select();
						});
					});
...
jQuery('#jigPresetSelect').val('c".$custom_preset_id."');});</script>";
			...
			add_settings_field(
				...
				array($this, 'jig_print_pass_input'),				// Field's callback
		...
		function jig_print_flickr_desc(){
			echo '<div id="jig_flickr" class="jigLong"><p>'.
				...
				'</p></div><p>'.
				__("Once you have added some users (at least one), you'll be able to load images from their Photostream or Favorites. You can also access the Groups, Galleries, Photosets and Collections belonging to a user. To actually select what content to display go to the Shortcode Editor, accessible when you edit a page or post.", 'jig_td').
				'</p></div>'; 
		}
		...
		function jig_print_pass_input($args){
			extract($args);
			echo '<input type="password" autocomplete="off" id="'.$id.'" name="'.self::SETTINGS_NAME.'['.$id.']" value="'.$this->settings[$id].'" /><div class="jigContextHelp" id=jigContext-'.$id.'>'.$label.'</div>';
		}
		...
		function jig_print_fb_authed(){
			...
			$output .= '<div id="jigFbAuthOtherUserPanel">
							<ol>
								...
								<li>'.__('Send him/her this link', 'jig_td').':<br /><input type="text" id="jigFbOtherUserLink" value="" data-force="https://www.facebook.com/v2.2/dialog/oauth?client_id='.trim($this->settings['fb_app_id']).'&scope=user_photos,read_stream' . '&redirect_uri='.urlencode(plugins_url('fb-auth-other-user.php', __FILE__)).'" /></li>
		...
		*/
		...
		function jig_init_shortcode($atts){
			...
			switch($gallery_type){
				case 'wp_post_gallery':
					...
					if($ids !== ''){ // if there is a list of image ids
						...
						if(empty($attachments)){
							...
							if($args['tax_query'][0]['taxonomy'] == $image_taxonomy){
								$args['tax_query'] = array(
														'relation' => 'AND',
														$args['tax_query'][0]
													);
				...
				case 'wp_recent_posts':
					...
					$is_wpml = function_exists('wpml_get_language_information');
				...
				case 'facebook':
					if(isset($this->settings['fb_authed'][$facebook_id])){
						...
						if($facebook_album == 'overview' || $facebook_album == 'overview_only_albums'){
							if($fb_lightbox_album == 'yes'){
								if(empty($facebook_album_multi) || (!empty($facebook_album_multi) && $fb_album_exclude == 'yes')){ // Preventing showing everything at once with lightbox albums.
									$preliminary_count_check_albums_url = 'https://graph.facebook.com/v2.2/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(count)&access_token='.$token;
								...
								$albums_url = 'https://graph.facebook.com/v2.2/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit('.$auto_limit.').fields(images,source,height,width,name))&access_token='.$token;
							}else{
								$albums_url = 'https://graph.facebook.com/v2.2/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
							}
							...
							if(!empty($facebook_album_multi) && $facebook_album_multi[0] == 'latestone'){
								...
								$albums_url = 'https://graph.facebook.com/v2.2/'.$facebook_id.'?fields=albums.limit(5).fields(id,cover_photo,link,count,name,description,type,photos.limit('.$limit.').fields(images,source,height,width,name))&access_token='.$token;
							...
							if(!empty($albums) && empty($albums['message'])){
								...
								if(!empty($album_name)){		
									...
									if($fb_breadcrumb == 'yes'){
										...
										$notice_before .= '<div class="jig-fbBreadcrumb"><a href="'.$fb_home_permalink.'">'.($fb_bc_home_text === '' ? $user['user_name'] : $fb_bc_home_text).'</a> '.$fb_bc_separator.' '.$album_name.'</div>';
									...
									foreach ($albums as $key => $album) {
										...
										if(!empty($album->count) && !empty($album->link) && !empty($album->photos->data)){
											if($fb_lightbox_album == 'no' && $fb_actual_cover_photo == 'yes' && !empty($album->cover_photo)){
												...
												$fb_cover_photo_url = 'https://graph.facebook.com/v2.2/'.$album->cover_photo.'?fields=images&access_token='.$token;
						...
						if($facebook_album == 'feed'){
							...
							$feed_url = 'https://graph.facebook.com/v2.2/'.$facebook_id.'/feed?fields=picture,caption,description,message,object_id'.$limit_parameter.'&access_token='.$token;
							...
							if(!empty($photos)){
								...
								foreach ($photos as &$feed_post) {
									if(!(isset($feed_post->picture) && isset($feed_post->object_id))){
										...
										$photo_object_url = 'https://graph.facebook.com/v2.2/'.$feed_post->object_id.'?fields=images,source,height,width&access_token='.$token;
						...
						if($facebook_album !== 'overview' && $facebook_album !== 'overview_only_albums' && $facebook_album !== 'feed'){
									...
									foreach ($albums as $key => $album) {
										if($album->type == 'normal' && !empty($album->count)){
											...
											if(($facebook_description == 'yes' || $facebook_description == 'above') && !empty($album->name)){
												$facebook_description_displayed_already = true;
												$notice_before .= '<p class="jig-fbDescription"><strong>'.$album->name.'</strong><br/>'.$album->description.'</p>';
											}
											break;
										}
									}
							...
							$photos_url = "https://graph.facebook.com/v2.2/".$facebook_album."/photos?fields=source,height,width,name".($facebook_image_size == 'normal' ? '' : ',images').$limit_parameter.'&access_token='.$token;
							...
							if(!empty($photos) && empty($photos['message'])){
								...
								if(($facebook_description == 'yes' || $facebook_description == 'above') && empty($facebook_description_displayed_already)){
									$facebook_album_url = "https://graph.facebook.com/v2.2/".$facebook_album."?fields=description".'&access_token='.$token;
				...
				case 'flickr':
					...
					if($flickr_collection !== ''){
						...
						if($flickr_caching > 0){
							if(get_transient('jigfli_'.md5($collections_url.$flickr_caching)) == true){
								...
								$collections_raw = unserialize($this->file_get_contents_curl($collections_url));
						...
						if(!empty($collections_raw) && $collections_raw['stat'] == 'ok'){
							...
							if($flickr_set_list_needed === true){
								$flickr_set_list_url = 'https://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method=flickr.photosets.getList&user_id='.$flickr_user.'&per_page=500&primary_photo_extras=description,url_k,url_h,url_l,url_c,url_z,url_m,url_n,url_s,url_t';
								if($flickr_caching > 0){
									if(get_transient('jigfli_'.md5($flickr_set_list_url.$flickr_caching)) == true){
										...
										$flickr_set_list = unserialize($this->file_get_contents_curl($flickr_set_list_url));
							...
							if($flickr_breadcrumb == 'yes' && !empty($collection_var)){
								...
								if(!empty($flickr_collection_ids)){
									foreach ($flickr_collection_ids as $flickr_collection_id_index => $flickr_collection_id) {
										if($flickr_collection_id !== end($flickr_collection_ids) || !empty($flickr_photoset)){				
											...
											$flickr_breadcrumb_inner .= $flickr_bc_separator.' <a href="'.$url.'">';
								...
								$notice_before .= '<div class="jig-flickrBreadcrumb"><a href="'.$flickr_home_permalink.'">'.($flickr_bc_home_text === '' ? $collections[$flickr_collection]['title'] : $flickr_bc_home_text).'</a> '.$flickr_breadcrumb_inner.'</div>';
							...
							if(!empty($collections[$flickr_collection]['set']) && count($collections[$flickr_collection]['set']) > 0){
								...
								foreach ($collections[$flickr_collection]['set'] as $single_set) {
									...
									if($flickr_lightbox_set == 'yes'){ // If gallery should be displayed as a lightbox
										...
										$photoset_url = 'https://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method=flickr.photosets.getPhotos&photoset_id='.$single_set['id'].$limit_parameter.'&extras=description,tags,geo,url_k,url_h,url_l,url_c,url_z,url_m,url_n,url_s,url_t';
										...
										if($flickr_caching > 0){
											if(get_transient('jigfli_'.md5($photoset_url.$flickr_caching)) == true){
												...
												$single_photoset = unserialize($this->file_get_contents_curl($photoset_url));
										...
										if(!empty($single_photoset) && $single_photoset['stat'] == "ok"){
											foreach ($single_photoset['photoset']['photo'] as $shadow_photo) {
												...
												$shadow_data = $sd = array(); // Create 2 arrays for this image one temporary and one that gets pushed
												$shadow_size = '';
												if($this->settings['flickr_allow_big_images'] === 'yes' && isset($shadow_photo['url_k'])){
													$shadow_data['url'] = $shadow_photo['url_k'];
													$shadow_data['width'] = $shadow_photo['width_k'];
													$shadow_data['height'] = $shadow_photo['height_k'];
												}elseif($this->settings['flickr_allow_big_images'] === 'yes' && isset($shadow_photo['url_h'])){
								...
								foreach ($collections[$flickr_collection]['collection'] as $single_collection) {
									...
									if($flickr_count == 'yes'){
										...
										if(!empty($single_collection['set']) && count($single_collection['set']) > 0){
											$description_fragments[] = count($single_collection['set']).' '._n('Set', 'Sets', count($single_collection['set']), 'jig_td');
					...
					$photos_url = 'https://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method='.$photos_url_bit.$limit_parameter.'&extras=description,tags,geo,url_k,url_h,url_l,url_c,url_z,url_m,url_n,url_s,url_t';
					if($flickr_search_geo == 1){
						$photos_url .= ',geo';
					}
					if($flickr_caching > 0){
								...
								if(!empty($enclosure_links)){
									...
									foreach ($enclosure_links as $enclosure_entry) {
										if(!empty($enclosure_entry)){
											...
											if(strpos($enclosure_link, 'gravatar') === false // Skip small gravatars
												&& $enclosure_link !== '//?#' // Weirdest thing ever, but it fixes non-appearing youtube feeds
												&& preg_match('/\.(jpe?g|gif|png|bmp)/im', $enclosure_link)){ // Test for images
			...
			foreach ($this->images as $image_index => &$image_element) {
				...
				if($link_target == 'off' && !($gallery_type == 'facebook' && $image_element['link_target'] == 'video')){
					...
					if($image_element['link_target'] == 'videoplayer'){
						...
						if((strpos($image_element['link'], 'youtube.com/') !== false || strpos($image_element['link'], 'vimeo.com/') !== false)
							&&
					...
					if(($lightbox == 'foobox' || $lightbox == 'magnific') && ($image_element['link_target'] == 'video' || $image_element['link_target'] == 'videoplayer')){
						if ($image_element['link_target'] == 'video' && preg_match('%https?://[^/\s]+/\S+\.(jpe?g|png|[tg]iff?|svg|bmp)%m', $image_element['link'])) {
				...
				if(!isset($image_element['photon'])){
					...
					$ext = '';
					if (preg_match('/.*\.(jpe?g|gif|bmp|webp)/im', $image_src, $regs)) {
						$ext = "&f=.".$regs[1];
					}
				...
				if($this->settings['show_up_in_feeds'] == 'yes' && is_feed()){
					...
					if($photon_activated && empty($skip_photon_for_animated_gif)){
						...
						if($use_timthumb == 'yes'){
							$ext = '';
							if (preg_match('/.*\.(jpe?g|gif|bmp|webp)/im', $image_rss_src, $regs)) {
								$ext = "&f=.".$regs[1];
							}
					...
					$rss_output .= '<img src="'.str_replace('&','&amp;',$image_rss_src).'" width="150" /> '; // Height will be automatic
			...
			switch($lightbox){
				case 'prettyphoto':
        			if(!defined('JIG_SKIP_PRETTYPHOTO')){
        				global $jig_prettyphoto_activation_needed, $jig_pinterest_trick_needed;
						$jig_prettyphoto_activation_needed = true;
						if(stripos($pp_social_buttons,'p') !== false){
							$jig_pinterest_trick_needed = true;
						}
				...
				case 'photoswipe':
					...
					global $jig_pswp_added;
					if(empty($jig_pswp_added)){
						$output .= '<div class="pswp" tabindex="-1" role="dialog" aria-hidden="true"><div class="pswp__bg"></div><div class="pswp__scroll-wrap"><div class="pswp__container"><div class="pswp__item"></div><div class="pswp__item"></div><div class="pswp__item"></div></div><div class="pswp__ui pswp__ui--hidden"><div class="pswp__top-bar"><div class="pswp__counter"></div><button class="pswp__button pswp__button--close" title="Close (Esc)"></button><button class="pswp__button pswp__button--share" title="Share"></button><button class="pswp__button pswp__button--fs" title="Toggle fullscreen"></button><button class="pswp__button pswp__button--zoom" title="Zoom in/out"></button><div class="pswp__preloader"><div class="pswp__preloader__icn"><div class="pswp__preloader__cut"><div class="pswp__preloader__donut"></div></div></div></div></div><div class="pswp__share-modal pswp__share-modal--hidden pswp__single-tap"><div class="pswp__share-tooltip"></div></div><button class="pswp__button pswp__button--arrow--left" title="Previous (arrow left)"></button><button class="pswp__button pswp__button--arrow--right" title="Next (arrow right)"></button><div class="pswp__caption"><div class="pswp__caption__center"></div></div></div></div></div>';
					}
					$jig_pswp_added = true;
				break;
				case 'photoswipe3':
			...
			global 	$justified_image_grid_js,
					...
					$jig_pinterest_trick_needed;
			$justified_image_grid_js .= $instance_js;

			$justified_image_grid_css .= $instance_css;
			$justified_image_grid_ie_css .= ($this->settings['center_filter_buttons'] == 'yes' ? ".jig-filterButton{display:inline !important;}" : "").
											($this->settings['center_tag_cloud'] == 'yes' ? ".jig-filterTag{display:inline !important;}" : "").
											$this->jig_rgbaIE($caption_bg_color, $jig_id, $caption_match_width, $gradient_caption_bg);
			...
			if(isset($jig_pinterest_trick_needed)){
				...
				// If the build function is not defined, try to remove the Pin scripts
				// delete the global vars it created
				// Remove its JS (pinit and pinit_main as well)
				// Also remove the CSS it leaves behind
				...
				$js_print = '(function() {
								...
								window["jigPinterestTrick"] = function(){
									if(typeof window["jigPinterestTrickRan"] !== "undefined"){
										window["jigPinterestTrickRan"]++;
										'.($this->settings['conditional_script_loading'] == 'no' ? 'if(window["jigPinterestTrickRan"] > 2){return;}' : 'return;').'
									}else{
									...
									if(foundFunction == false){ 
										for(aProp in window){
											try{
												if(ownProp(window, aProp)){
													if((typeof window[aProp] === "boolean" || typeof window[aProp] === "object") && aProp.indexOf("PIN_") === 0){
														delete window[aProp];											
														$(\'script[src*="assets.pinterest.com/js/pinit"]\').remove();
														$(\'style[type="text/css"]:contains(\'+aProp+\')\').remove();
													}
												}
											}catch(exception){}
										}
								...
								$(window).load(window["jigPinterestTrick"]);
							})();'.$js_print;
			}
			...
			$this->dynamic_style = "<style type='text/css'>\r\n".str_replace(array("\r\n", "\r", "\n", "\t", '  ', '    ', '    '), '', $css_print)."\r\n</style>\r\n".($justified_image_grid_ie_css ? "<!--[if IE]><style type='text/css'>".
										str_replace(array("\r\n", "\r", "\n", "\t", '  ', '    ', '    '), '', $justified_image_grid_ie_css.'.jig-caption-wrapper-clone {filter: alpha(opacity=0) !important;}').
										'</style><![endif]-->' : '')."<!--[if lt IE 8]><style type='text/css'>.justified-image-grid .jig-overlay, .justified-image-grid .jig-overlay-icon-wrapper, .justified-image-grid .jig-overlay-icon {position:absolute;top:0;right:0;bottom:0;left:0;} .justified-image-grid .jig-overflow, .jig-overflow div {cursor: pointer;} .jig-caption-wrapper{margin:0 !important;} </style><![endif]-->\r\n";
			if($this->settings['conditional_script_loading'] == 'yes'){
		...
		function jig_fb_auth(){
			...
			if(empty($_REQUEST["code"])) {
				...
				$dialog_url = "https://www.facebook.com/v2.2/dialog/oauth?client_id=" 
				. $app_id . "&scope=user_photos,read_stream" . "&redirect_uri=" . urlencode($redirect_uri) . "&state="
				. $jig_session['state'];
			...
			if($jig_session['state'] && ($jig_session['state'] === $_REQUEST['state'])) {
				...
				$graph_url = "https://graph.facebook.com/v2.2/me?fields=name,picture&access_token=" 
				. $params['access_token'];
			    ...
			    $albums_url = "https://graph.facebook.com/v2.2/me/albums?fields=id,link,count,from&limit=1000&access_token=".$params['access_token'];
		...
		function jig_get_fb_access_token(){
			...
			if($code == 'current'){
					$jig_session = get_transient('jig_session');
					if(isset($jig_session['fb_details'])){
		...
		function jig_add_fb_page($token = '', $user_name = ''){
			...
			if($page != ''){
				$albums_url = "https://graph.facebook.com/v2.2/".$page.'/albums?fields=id,link,count,from&limit=1000&access_token='.$token;
				...
				if(!empty($albums) && empty($albums['message'])){
					...
					if($found > 0){
						$user_url = "https://graph.facebook.com/v2.2/".$page.'?fields=name,picture&access_token='.$token;
					...
					$main_url = "https://graph.facebook.com/v2.2/".$page.'?access_token='.$token;
		...
		function jig_verify_fb_authed(){
			...
			$albums_url = "https://graph.facebook.com/v2.2/".$user_id.'/albums?fields=id,link,count,from&limit=1000&access_token='.$token;
			$albums = $this->facebook_api_call($albums_url, 0, 1000);
			...
			if(!empty($albums) && empty($albums['message'])){
				...
				if($found > 0){
					$user_url = "https://graph.facebook.com/v2.2/".$user_id.'?fields=name,picture&access_token='.$token;
					$user = json_decode($this->file_get_contents_curl($user_url));
					$output = array(	
						'user_name' => $user->name,
						'picture' => ($this->settings['timthumb_path'] ?
		...
		function jig_get_fb_albums(){
			...
			$albums_url = "https://graph.facebook.com/v2.2/".$user_id.'?fields=albums.limit(1000).fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
		...
		function jig_get_fli_types(){
			...
			// check for groups
			$flickr_url = "https://api.flickr.com/services/rest?api_key=".$fli_api_key."&format=php_serial&method=flickr.people.getPublicGroups&user_id=".$user_id;
			$rsp = unserialize($this->file_get_contents_curl($flickr_url));
			...
			if($rsp['photosets']['total'] > 0){
				// got photosets
				$output['elements'] .= '<div class="JIGupdateButton fliTypeBtn" id="fliPhotosetSelector">Photoset (+)</div>';
			}
		...
		function flickr_collection_walk($collection,$fli_api_key,$depth){
			...
			if(!empty($collection['collection']) && count($collection['collection']) > 0){
				...
				foreach ($collection['set'] as $set) {
					...
					$set_rsp = unserialize($this->file_get_contents_curl($flickr_url));
		...
		function jig_purge_rss_caching(){
			...
			if($num !== false){
				$output['result'] = sprintf(__('RSS Cache purged, count: %d.','jig_td'),$num);
			}else{
				$output['result'] = __('Error purging the cache.','jig_td');
			}
			echo json_encode($output);
